...
import { Position, PositionError } from './xiangqi.js';
...
export const makePgn = (game: Game<PgnNodeData>): string => {
  ...
  const initialPly = fen
    ? parseFen(fen).unwrap(
      setup => (setup.fullmoves - 1) * 2 + (setup.turn === 'red' ? 0 : 1),
      _ => 0,
    )
    : 0;
...
export const parseVariant = (variant: string | undefined): Rules | undefined => {
  switch ((variant || 'xiangqi').toLowerCase()) {
    case 'xiangqi':
    case 'standard':
    case 'from position':
    case 'classical':
